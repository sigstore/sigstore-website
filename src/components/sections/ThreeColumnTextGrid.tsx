...
interface ThreeColumnTextGridProps {
  header?: string;
  subHeader?: string;
  text?: string;
  column1?: ColumnData;
...
export function ThreeColumnTextGrid({
  header,
  subHeader,
  text,
  column1,
  ...
  return (
    <section
      className={`py-64 md:py-128 md:px-16 ${
      ...
      <div className="container">
        ...
        <div
          className={`md:flex items-start md:-ml-30 md:-mr-30 md:pr-30 ${
            column3?.header ? "justify-between" : "justify-start"
          }`}
        >
          {column1 && <Card {...column1} slug={slug} />}
          {column2 && <Card {...column2} slug={slug} />}
          {column3 && <Card {...column3} slug={slug} />}
        </div>
      </div>
    </section>
  );
